...
import { deleteFileFromCloudinary, emitEvent } from "../utils/features.js";
import {
  ALERT,
  REFETCH_CHAT,
  NEW_ATTACHMENT,
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, REFETCH_CHAT, chat, chat.members);
  return res.status(200).json({
    success: true,
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  if (!chat.groupChat)
    return next(new ErrorHandler("This is not a chat group", 400));
  ...
  return res.status(200).json({
    success: true,
    message: "Member Leave Group successfully",
  });
});
...
const sendAttatchments = TryCatch(async (req, res, next) => {
  ...
  //Upload files here on Cloudnery
  const attachments = [];
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  // TODO: Delete all chats and attachments/files form cloudnaryi
